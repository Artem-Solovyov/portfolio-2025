      ...
      <div className="flex gap-4 lg:gap-6 justify-evenly flex-wrap">
        {works.map((work) => (
          ...
          >
            ...
            <CardFooter className="justify-between  bg-black/50 border-white/40 border-1 overflow-hidden py-3 absolute before:rounded-xl rounded-large bottom-1 w-[calc(100%_-_16px)] shadow-small ml-2 z-10 mb-1 flex gap-2 ">
              <p className="text-sm text-white/80 ">{work.desc}</p>
              <Link
                isExternal
                target="_blanc"
                className="text-sm text-white bg-white/20 p-3 hover:bg-white/90 hover:rotate-45 transition-all rounded-full  hover:text-primary"
                href={work.link}
              >
